...
const Signup = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.identifier.trim()) {
      newErrors.identifier = translate("auth.emailOrPhoneRequired");
    } else {
      const isEmailFormat = isEmail(formData.identifier);
      const isPhoneFormat = isPhone(formData.identifier);
      
      if (!isEmailFormat && !isPhoneFormat) {
        newErrors.identifier = translate("auth.invalidEmailOrPhone");
      }
    }

    if (!formData.password) {
      newErrors.password = translate("auth.passwordRequired");
    } else if (formData.password.length < 8) {
      newErrors.password = translate("auth.passwordMinLength");
    }

    if (!formData.confirmPassword) {
      newErrors.confirmPassword = translate("auth.confirmPasswordRequired");
    } else if (formData.password !== formData.confirmPassword) {
      newErrors.confirmPassword = translate("auth.passwordsDoNotMatch");
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!validateForm()) {
      toast.error(translate("auth.pleaseFixErrors"));
      return;
    }
  ...
  return (
    <>
      <Breadcrumb title={translate("auth.signUp")} pages={[translate("auth.signUp")]} />
      <section className="overflow-hidden py-20 bg-gray-2">
        <div className="max-w-[1170px] w-full mx-auto px-4 sm:px-8 xl:px-0">
          <div className="max-w-[570px] w-full mx-auto rounded-xl bg-white shadow-1 p-4 sm:p-7.5 xl:p-11">
            <div className="text-center mb-11">
              <h2 className="font-semibold text-xl sm:text-2xl xl:text-heading-5 text-dark mb-1.5">
                {translate("auth.createAccountTitle")}
              </h2>
              <p>{translate("auth.enterDetailsBelow")}</p>
            </div>
            ...
            <div className="flex flex-col gap-4.5 mb-6">
              ...
              >
                ...
                {translate("auth.signUpWithGoogle")}
              </button>
              ...
              >
                ...
                {translate("auth.signUpWithGithub")}
              </button>
            </div>

            <span className="relative z-1 block font-medium text-center mt-4.5">
              <span className="block absolute -z-1 left-0 top-1/2 h-px w-full bg-gray-3"></span>
              <span className="inline-block px-3 bg-white">{translate("auth.or")}</span>
            </span>
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-5">
                  <label htmlFor="identifier" className="block mb-2.5">
                    {translate("auth.emailOrPhone")} <span className="text-red">*</span>
                  </label>
                  <input
                    type="text"
                    name="identifier"
                    ...
                    placeholder={translate("auth.emailOrPhonePlaceholder")}
                    className={`rounded-lg border ${
                      errors.identifier ? "border-red" : "border-gray-3"
                  ...
                  />
                  {errors.identifier && (
                    <p className="text-red text-sm mt-1.5">{errors.identifier}</p>
                  )}
                </div>

                <div className="mb-5">
                  <label htmlFor="password" className="block mb-2.5">
                    {translate("auth.password")} <span className="text-red">*</span>
                  </label>
                  <input
                    type="password"
                    name="password"
                  ...
                  />
                  {errors.password && (
                    <p className="text-red text-sm mt-1.5">{errors.password}</p>
                  )}
                </div>

                <div className="mb-5.5">
                  <label htmlFor="confirmPassword" className="block mb-2.5">
                    {translate("auth.reTypePassword")} <span className="text-red">*</span>
                  </label>
                  <input
                    type="password"
                    ...
                    onChange={handleInputChange}
                    placeholder={translate("auth.reTypePasswordPlaceholder")}
                    autoComplete="new-password"
                    className={`rounded-lg border ${
                  ...
                  {errors.confirmPassword && (
                    <p className="text-red text-sm mt-1.5">{errors.confirmPassword}</p>
                  )}
                </div>
      ...
      <OTPModal
        ...
        phoneOrEmail={formData.identifier}
        isLoading={isLoading}
        error={otpError}
